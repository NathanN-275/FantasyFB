...
export interface EspnCompanionDraftEventV1 {
  readonly version: 1;
  readonly eventId: string;
  readonly draftId: string;
  readonly eventType: NewDraftEvent["eventType"];
...
/** Normalizes trusted future companion messages; this does not scrape ESPN. */
export function normalizeEspnCompanionEvent(message: EspnCompanionDraftEventV1): NewDraftEvent {
  if (message.version !== 1) throw new Error("Unsupported ESPN companion event version.");
  return newDraftEventSchema.parse({
    eventId: message.eventId,
    draftId: message.draftId,
    source: "espn_companion",
    eventType: message.eventType,
    ...(message.overallPick === undefined ? {} : { overallPick: message.overallPick }),
    ...(message.round === undefined ? {} : { round: message.round }),
    ...(message.draftSlot === undefined ? {} : { draftSlot: message.draftSlot }),
    ...(message.fantasyTeamId === undefined ? {} : { fantasyTeamId: message.fantasyTeamId }),
    ...(message.playerId === undefined ? {} : { playerId: message.playerId }),
    ...(message.playerExternalId === undefined
      ? {}
      : { playerExternalId: message.playerExternalId }),
    ...(message.keeperStatus === undefined ? {} : { keeperStatus: message.keeperStatus }),
    providerTimestamp: message.occurredAt,
    ...(message.correctionReference === undefined
      ? {}
      : { correctionReference: message.correctionReference })
  });
}
